    ...
    <aside className="fixed left-0 top-0 h-screen w-64 bg-zinc-900 border-r border-zinc-800 flex flex-col z-50">
      ...
      <div className="p-6 border-b border-zinc-800">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ backgroundColor: '#8B0000' }}>
            <span className="text-white font-bold text-sm">PH</span>
          </div>
          <div>
            <p className="font-bold text-zinc-50 text-sm leading-tight">PHM Sistema</p>
            <p className="text-zinc-500 text-xs">PHMavericks Agency</p>
          </div>
        </div>
      </div>
        ...
        {visibleNav.map((item) => {
          ...
          return (
            <Link
              ...
              style={isActive ? { backgroundColor: '#8B0000', color: 'white' } : {}}
            >
              {item.icon}
              {item.label}
            </Link>
