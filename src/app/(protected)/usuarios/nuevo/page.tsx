  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/usuarios', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, verificationCode }),
      })
      if (res.ok) {
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-zinc-500 text-sm">
            {step === 'form' ? 'Crea una cuenta de acceso al sistema' : 'Ingresa el código enviado al correo'}
          </p>
        </div>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
        ...
        {step === 'form' && (
          <form onSubmit={handleSendCode} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-300 mb-1.5">Contraseña * (mínimo 6 caracteres)</label>
          ...
          <form onSubmit={handleCreateUser} className="space-y-5">
            <div className="text-center pb-2">
              ...
              <p className="text-zinc-300 text-sm">
                Se envió un código de 6 dígitos a <strong className="text-zinc-100">{form.email}</strong>
              </p>
            </div>
            ...
            {devMode && (
              <div className="flex items-start gap-2 px-4 py-3 bg-amber-950/50 border border-amber-800/50 rounded-lg">
                ...
                <p className="text-xs text-amber-400 leading-relaxed">
                  <strong>Modo desarrollo:</strong> No hay SMTP configurado. Revisa la consola del servidor para ver el código.
                </p>
              </div>
            )}
