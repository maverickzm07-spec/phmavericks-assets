    ...
    <div className="max-w-3xl space-y-5">
      ...
      <PremiumCard padding="md">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex-shrink-0">
            ...
            <select
              value={editEstado}
              onChange={(e) => { setEditEstado(e.target.value); updateEstado(e.target.value) }}
              disabled={saving}
        ...
        {contents.length > 0 && (
          <div className="mt-4 pt-4 border-t border-phm-border-soft">
            <div className="flex justify-between text-xs text-phm-gray-soft mb-2">
              ...
              <span className={`font-semibold ${pct === 100 ? 'text-emerald-400' : 'text-phm-gold'}`}>
                {pct}%
              </span>
            </div>
      ...
      <PremiumCard padding="md">
        ...
        <DeliverySection
          ...
          entityType="PROJECT"
          entityId={project.id}
          existingAccess={(project as any).deliveryAccesses?.[0] ?? null}
        />
      </PremiumCard>

      {/* Entregables (solo producción interna) */}
              ...
              <div key={c.id} className="p-4 flex items-center gap-3 row-hover">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {c.formato && (
                      <span className="text-xs text-phm-gray-soft">· {getFormatoLabel(c.formato)}</span>
                    )}
                    {c.driveLink && (
      ...
      {error && (
        <div className="px-4 py-3 bg-red-950/60 border border-red-900/60 rounded-lg text-sm text-red-300">
          {error}
        </div>
      )}
    </div>
