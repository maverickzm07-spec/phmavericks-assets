...
function buildWhatsappUrl(clientName: string, subject: string): string {
  const msg = encodeURIComponent(`Hola, soy ${clientName}. Tengo una consulta sobre mi entrega: ${subject}.`)
  return `https://wa.me/${PHM_WHATSAPP}?text=${msg}`
}

interface StatusCfg { label: string; color: string; bg: string; border: string; icon: 'check' | 'clock' | 'alert' | 'progress' }
...
export default function EntregaSlugPage() {
  ...
  if (error || !data) return (
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center px-4 text-center gap-4">
      ...
      <p className="text-zinc-500 text-sm">{error || 'Este enlace no existe o ha sido desactivado.'}</p>
    </div>
  )
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <header className="sticky top-0 z-10 bg-zinc-950/90 backdrop-blur-xl border-b border-zinc-800/60">
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <Image src="/logo-phmavericks.png" alt="PHMavericks" width={32} height={32} className="rounded-lg object-contain" priority />
            <div>
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-4 pb-20">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
          ...
          {client.business && <p className="text-zinc-400 text-sm mt-0.5">{client.business}</p>}
          ...
          <div className="mt-4 pt-4 border-t border-zinc-800/80 flex flex-wrap items-start gap-x-6 gap-y-3">
            ...
            {project?.fechaEntrega && (
              <div>
                ...
                <p className="font-semibold text-white text-sm">
                  {new Date(project.fechaEntrega).toLocaleDateString('es-EC', { day: '2-digit', month: 'long', year: 'numeric' })}
                </p>
              </div>
            )}
          </div>
        ...
        <div className={`${statusCfg.bg} border ${statusCfg.border} rounded-2xl p-5`}>
          ...
          {project?.observaciones && (
            <p className="mt-3 text-sm text-zinc-400 border-t border-zinc-700/50 pt-3">{project.observaciones}</p>
          )}
        </div>
          ...
          {deliveryLink ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2 bg-zinc-800/70 rounded-xl px-3 py-2.5 border border-zinc-700/50">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4 shrink-0 text-[#C9A84C]">
                  <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" />
                  <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" />
                </svg>
                <p className="text-xs text-zinc-300 font-mono truncate flex-1 select-all">{deliveryLink}</p>
              </div>
                ...
                <div className="mt-1">
                  ...
                  <div className="rounded-xl overflow-hidden border border-zinc-700/50 bg-zinc-800/50">
                    <iframe
                      ...
                      className="w-full"
                      style={{ height: embedInfo.type === 'folder' ? 620 : 480, border: 'none' }}
                      allow="autoplay"
              ...
              {embedInfo.embedUrl && previewFailed && (
                <p className="text-xs text-zinc-600 text-center pt-1">La vista previa no está disponible para este enlace.</p>
              )}
              {!embedInfo.embedUrl && (
                <p className="text-xs text-zinc-600 text-center pt-1">La vista previa no está disponible para este tipo de enlace.</p>
              )}
            </div>
          ) : (
